const Sync={
  ...
  async createCloudUser({name,email,password,role='teacher',gradeAccess={},coachTeams={},coachingGroups={}}){if(!this.enabled()||!Auth.isAdmin())throw new Error('Nur ein angemeldeter Admin kann Konten anlegen.');const temp=window.supabase.createClient(Auth.cloud.url,Auth.cloud.anonKey,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}});const {data,error}=await temp.auth.signUp({email,password,options:{data:{display_name:name}}});if(error)throw new Error(error.message||'Konto konnte nicht angelegt werden.');const id=data?.user?.id;if(!id)throw new Error('Supabase hat kein Benutzerkonto zurückgegeben.');await new Promise(r=>setTimeout(r,250));const patch={display_name:name,role:role||'teacher',active:false,coach_teams:coachTeams||{},coaching_groups:coachingGroups||{}};const {error:pErr}=await Auth.cloudClient.from('kompass_profiles').update(patch).eq('id',id);if(pErr)throw new Error('Konto wurde erstellt, aber die Zuordnung konnte nicht gespeichert werden: '+pErr.message);for(const [grade,level] of Object.entries(gradeAccess||{})){const {error:gErr}=await Auth.cloudClient.from('kompass_grade_access').upsert({user_id:id,grade:Number(grade),access_level:level});if(gErr)throw new Error('Konto wurde erstellt, aber das Stufenrecht konnte nicht gespeichert werden: '+gErr.message);}Store.log('Cloud-Benutzer angelegt',{target:name,email,role,gradeAccess,coachTeams,coachingGroups});return {user:{id,email},profile:patch};},
