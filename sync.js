const Sync={
  ...
  async cloudProfiles(){if(!this.enabled())return[];const [{data:profiles,error:pErr},{data:access,error:aErr}]=await Promise.all([Auth.cloudClient.from('kompass_profiles').select('id,display_name,role,active,coach_teams,created_at').order('display_name'),Auth.cloudClient.from('kompass_grade_access').select('user_id,grade,access_level')]);if(pErr)throw pErr;if(aErr)throw aErr;return (profiles||[]).map(p=>({...p,gradeAccess:Object.fromEntries((access||[]).filter(a=>a.user_id===p.id).map(a=>[a.grade,a.access_level]))}));},
  async createCloudUser({name,email,password,role='teacher',gradeAccess={},coachTeams={}}){if(!this.enabled()||!Auth.isAdmin())throw new Error('Nur ein angemeldeter Admin kann Konten anlegen.');const {data,error}=await Auth.cloudClient.functions.invoke('kompass-admin-create-user',{body:{name,email,password,role,gradeAccess,coachTeams}});if(error)throw new Error(error.message||'Konto konnte nicht angelegt werden.');if(data?.error)throw new Error(data.error);Store.log('Cloud-Benutzer angelegt',{target:name,email,role,gradeAccess,coachTeams});return data;},
