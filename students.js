...
function students(){
  ...
  let content = header('Schüler','Alphabetische Schüleransicht mit Coach, Team, Verlauf und LEB-Entwurf.');
...
function studentProfile(){
  ...
  content += entries.length ? entries.map(([key,r])=>{
    ...
    return `<div class="card"><b>${comp ? comp.text : compId}</b><p>${comp ? comp.subject+' · '+(comp.sprint?'Sprint '+comp.sprint:'Prozess')+' · Niveau '+comp.level : ''}</p><p>${statusLabel(r.current)} · erstmals erreicht: ${r.firstGreen || '—'}</p><div class="mini">${r.history.length} Bewertung(en), ${(r.notes||[]).length} Notiz(en)</div></div>`;
...
function pupilEditForm(p){
  return `<div class="card">
    ...
    <div class="formgrid">
      ...
      <div><label>Niveau</label><select id="p_level">
        ${['G','M','E'].map(n=>`<option ${p.level===n?'selected':''}>${n}</option>`).join('')}
      </select></div>
    </div>
...
function savePupilProfile(){
  ...
  p.level = document.getElementById('p_level').value;
