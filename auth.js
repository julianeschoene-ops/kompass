...
const Auth={
  ...
  ensureLocalBootstrap(){
    let db;try{db=JSON.parse(localStorage.getItem(AUTH_KEY)||'null')}catch(e){db=null}
    if(!db||!Array.isArray(db.users)){
      db={users:[],createdAt:new Date().toISOString()};localStorage.setItem(AUTH_KEY,JSON.stringify(db));
    }
  },
  ...
  currentUser(){return this.session?.user||null},
  isLoggedIn(){return !!this.currentUser()},
  isAdmin(){return this.currentUser()?.role==='admin'},
  canLead(){return ['admin','leitung'].includes(this.currentUser()?.role)},
  ...
  async createFirstAdmin(name,username,password){
    const db=this.localDb();if(db.users.length)throw new Error('Es existiert bereits ein Konto.');
    const u={id:uid('user'),name:name.trim(),username:username.trim().toLowerCase(),passwordHash:await this.hash(password),role:'admin',active:true,createdAt:new Date().toISOString()};db.users.push(u);this.saveLocalDb(db);await this.localLogin(username,password);return u;
  },
  async createLocalUser({name,username,password,role}){
    const db=this.localDb();username=username.trim().toLowerCase();if(db.users.some(u=>u.username===username))throw new Error('Benutzername existiert bereits.');
    const u={id:uid('user'),name:name.trim(),username,passwordHash:await this.hash(password),role:role||'teacher',active:true,createdAt:new Date().toISOString()};db.users.push(u);this.saveLocalDb(db);Store.log('Benutzerkonto angelegt',{target:u.name,role:u.role});return u;
  },
  async localLogin(username,password){
    const db=this.localDb(),hash=await this.hash(password);const u=db.users.find(x=>x.active!==false&&x.username===username.trim().toLowerCase()&&x.passwordHash===hash);if(!u)throw new Error('Benutzername oder Passwort ist nicht korrekt.');
    this.session={mode:'local',user:{id:u.id,name:u.name,username:u.username,role:u.role}};sessionStorage.setItem(SESSION_KEY,JSON.stringify(this.session));return this.session.user;
  },
  ...
  async applyCloudSession(s){
    const {data,error}=await this.cloudClient.from('kompass_profiles').select('id,display_name,role,active').eq('id',s.user.id).single();if(error||!data||data.active===false){await this.cloudClient.auth.signOut();throw new Error('Dieses KOMPASS-Konto ist noch nicht freigeschaltet.');}
    this.session={mode:'cloud',user:{id:data.id,name:data.display_name||s.user.email,username:s.user.email,role:data.role||'teacher'}};sessionStorage.setItem(SESSION_KEY,JSON.stringify(this.session));State.teacher=this.session.user.name;State.role=this.session.user.role;if(window.Sync)await Sync.pull();render();
  },
