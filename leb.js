...
function generateProcessParagraph(pupil,processRows){if(!processRows.length)return ''; const green=processRows.filter(r=>r.record.current==='green'), yellow=processRows.filter(r=>r.record.current==='yellow'), red=processRows.filter(r=>r.record.current==='red'); const parts=[]; if(green.length) parts.push(`${pupil.first} zeigt im Bereich ${green.slice(0,2).map(r=>r.comp.area.toLowerCase()).join(' und ')} ein sicheres Arbeitsverhalten.`); if(yellow.length) parts.push(`In ${yellow.slice(0,2).map(r=>r.comp.area.toLowerCase()).join(' und ')} entwickelt ${pupil.first} zunehmend Sicherheit.`); if(red.length) parts.push(`Im Bereich ${red.slice(0,1).map(r=>r.comp.area.toLowerCase()).join(' und ')} benötigt ${pupil.first} noch Unterstützung.`); return parts.join(' ');}
function generateSubjectParagraph(pupil,subject,items){const levels=items.map(r=>r.record.level).filter(Boolean); const dominantLevel=mostCommon(levels)||'dem gewählten'; const green=items.filter(r=>r.record.current==='green'), yellow=items.filter(r=>r.record.current==='yellow'), red=items.filter(r=>r.record.current==='red'); let text=`Im Fach ${subject} arbeitet ${pupil.first} überwiegend auf ${dominantLevel}-Niveau. `; if(green.length) text += `${pupil.first} kann zentrale Anforderungen sicher bearbeiten. `; if(yellow.length) text += `In einzelnen Bereichen zeigt ${pupil.first} bereits sichere Ansätze und entwickelt zunehmend Sicherheit. `; if(red.length) text += `Bei einzelnen Anforderungen benötigt ${pupil.first} noch Unterstützung. `; if(hasProgress(items)) text += `Im Verlauf der Werkstattarbeit ist eine positive Entwicklung erkennbar.`; return text.trim();}
