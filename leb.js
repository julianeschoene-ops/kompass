...
function verbalizeCompetence(comp, status, pupil, level){
...
function generateLebText(pupil){
  ...
  Object.entries(bySubject).forEach(([subject,items])=>{
    ...
    if(sorted.length === 1){
      parts.push(`Im Fach ${subject}: ${verbalizeCompetence(sorted[0].comp, sorted[0].record.current, pupil, sorted[0].record.level)}`);
    } else {
      parts.push(`Im Fach ${subject} zeigt sich folgender Lernstand:`);
      sorted.forEach(r=>parts.push(verbalizeCompetence(r.comp, r.record.current, pupil, r.record.level)));
    }
  });
