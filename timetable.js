...
/* ============================================================
   KOMPASS 8.1a – Lehrkraftprofil 3.0
   ============================================================ */
const TT81_DEPLOYMENT_AREAS=['5','6','7','8','9','10','Oberstufe'];
const TT81_WORKSHOP_LABS=['Geo-Lab','History-Lab','IT-Lab','Nawi-Lab','Kunst-Lab','Musik-Lab','Technik-Lab','Sprachen-Lab'];
const TT81_DEFAULT_TEAMS={
  '5':['Blau','Rot','Gelb','Violett','Grün'],
  '6':['Blau','Rot','Gelb','Violett','Grün'],
  '7':['Blau','Rot','Gelb','Violett','Grün']
};
function tt81EnsureTeacher(x){
  x=tt80EnsureTeacher(x);
  x.coachStage=String(x.coachStage||'');
  x.coachTeamId=x.coachTeamId||'';
  x.coachTeamName=x.coachTeamName||'';
  x.coachSubteam=x.coachSubteam||'';
  x.coachingHours=Number.isFinite(Number(x.coachingHours))?Number(x.coachingHours):(x.coachStage?1.5:0);
  x.teamMeetingHours=Number.isFinite(Number(x.teamMeetingHours))?Number(x.teamMeetingHours):0.5;
  x.teamMeetingActive=x.teamMeetingActive!==false;
  x.freeDayWish=[0,1,2].includes(Number(x.freeDayWish))?Number(x.freeDayWish):0;
  x.laByStage=x.laByStage||{};
  for(const y of [5,6,7]){
    const previous=(y===6&&x.laPrefs)?x.laPrefs:{1:2,2:2,3:2};
    x.laByStage[y]=x.laByStage[y]||{eligible:y===6?x.laEligible!==false:true,prefs:{1:Number(previous[1]??2),2:Number(previous[2]??2),3:Number(previous[3]??2)}};
  }
  x.workshopLabs=Array.isArray(x.workshopLabs)?x.workshopLabs:[];
  x.workshopOwnLab=x.workshopOwnLab||'';
  x.workshopTandem=x.workshopTandem||'egal';
  return x;
}
function tt81ReservedTotal(x){x=tt81EnsureTeacher(x);return (x.coachStage?Number(x.coachingHours||0):0)+(x.teamMeetingActive?Number(x.teamMeetingHours||0):0)}
function tt81KompassTarget(x){x=tt81EnsureTeacher(x);return Math.max(0,tt80TeachingTarget(x)-tt80ExternalTotal(x)-tt81ReservedTotal(x))}
function tt81CoachOptions(stage){
  if(!stage)return [];
  const actual=tt().groups.filter(g=>String(g.year)===String(stage));
  if(actual.length)return actual.map(g=>({id:g.id,name:g.name}));
  return (TT81_DEFAULT_TEAMS[String(stage)]||[]).map(n=>({id:`virtual-${stage}-${n.toLowerCase()}`,name:`Team ${n}`}));
}
function tt81CoachStageChanged(){
  const stage=document.getElementById('tt81_coach_stage').value,sel=document.getElementById('tt81_coach_team'),old=sel.value;
  sel.innerHTML='<option value="">kein Farbteam</option>'+tt81CoachOptions(stage).map(g=>`<option value="${esc(g.id)}" data-name="${esc(g.name)}">${esc(g.name)}</option>`).join('');
  if([...sel.options].some(o=>o.value===old))sel.value=old;
  const sub=document.getElementById('tt81_subteam_wrap');if(sub)sub.hidden=!/violett/i.test(sel.options[sel.selectedIndex]?.text||'');
  tt81UpdateDeputatPreview();
}
function tt81CoachTeamChanged(){const sel=document.getElementById('tt81_coach_team'),sub=document.getElementById('tt81_subteam_wrap');if(sub)sub.hidden=!/violett/i.test(sel.options[sel.selectedIndex]?.text||'')}
function tt81UpdateDeputatPreview(){
  const full=Number(document.getElementById('tt80_full_dep')?.value||0),red=[...document.querySelectorAll('.tt80ReductionRow')].reduce((s,r)=>s+Number(r.querySelector('[data-red=hours]').value||0),0),ext=[...document.querySelectorAll('.tt80ExternalRow')].reduce((s,r)=>s+Number(r.querySelector('[data-extc=hours]').value||0),0),coach=Number(document.getElementById('tt81_coach_stage')?.value?document.getElementById('tt81_coaching_hours')?.value||1.5:0),team=document.getElementById('tt81_team_active')?.checked?Number(document.getElementById('tt81_team_hours')?.value||0.5):0,teach=Math.max(0,full-red),kom=Math.max(0,teach-ext-coach-team),el=document.getElementById('tt80_dep_preview');
  if(el)el.innerHTML=`<b>${full} h</b> volles Deputat − <b>${red} h</b> Ermäßigung = <b>${teach} h</b> Unterricht · − <b>${ext} h</b> außerhalb KOMPASS · − <b>${coach+team} h</b> Coaching/Team · <strong>${kom} h durch KOMPASS</strong>`;
}
function tt81LAPanel(x,year){const d=x.laByStage[year];return `<div class="tt81LACard"><label class="check tt79MasterCheck"><input id="tt81_la_${year}_eligible" type="checkbox" ${d.eligible?'checked':''}> In Stufe ${year} im Lernatelier einsetzbar</label><div class="formgrid">${[1,2,3].map(i=>`<div><label>LA ${i}</label><select id="tt81_la_${year}_${i}"><option value="3" ${Number(d.prefs[i])===3?'selected':''}>bevorzugt</option><option value="2" ${Number(d.prefs[i])===2?'selected':''}>möglich</option><option value="1" ${Number(d.prefs[i])===1?'selected':''}>nur bei Bedarf</option><option value="0" ${Number(d.prefs[i])===0?'selected':''}>nicht einsetzen</option></select></div>`).join('')}</div></div>`}
function ttRenderTeacherDialog(target){
 const x=tt81EnsureTeacher(tt78Teacher(State.dialog?.id)),groups=tt().groups.filter(g=>[5,6,7].includes(Number(g.year))),core=['Deutsch','Mathematik','Englisch'],red=tt80ReductionTotal(x),teach=tt80TeachingTarget(x),ext=tt80ExternalTotal(x),reserved=tt81ReservedTotal(x),kom=tt81KompassTarget(x),coachOptions=tt81CoachOptions(x.coachStage);
 target.innerHTML=`<div class="dialogBackdrop" onclick="if(event.target===this){State.dialog=null;renderDialog()}"><div class="dialog ttTeacherDialog tt80Dialog"><div class="dialogHead"><div><h2>${x.id?'Lehrkraftprofil bearbeiten':'Lehrkraft anlegen'}</h2><div class="mini">KOMPASS 8.1a · vollständige Planungsdaten</div></div><button class="iconBtn" onclick="State.dialog=null;renderDialog()">×</button></div>
 <div class="tt80Dashboard"><div><small>Volles Deputat</small><b>${x.fullDeputat} h</b></div><div><small>Ermäßigung</small><b>${red} h</b></div><div><small>Extern verplant</small><b>${ext} h</b></div><div><small>Coaching + Team</small><b>${reserved} h</b></div><div class="primary"><small>Durch KOMPASS</small><b>${kom} h</b></div></div>
 <details open><summary>1 · Deputat und feste Abzüge</summary><div class="tt80Section"><div class="formgrid"><div><label>Vorname</label><input id="tt_first" value="${esc(x.first||'')}"></div><div><label>Nachname</label><input id="tt_last" value="${esc(x.last||'')}"></div><div><label>Volles Deputat</label><input id="tt80_full_dep" type="number" min="0" step="0.5" value="${x.fullDeputat}" oninput="tt81UpdateDeputatPreview()"></div><div><label>Studierte Fächer</label><input id="tt_subj" value="${esc(x.subjects||'')}"></div><div><label>Weitere mögliche Fächer</label><input id="tt_other_subj" value="${esc(x.otherSubjects||'')}"></div></div><label>Ermäßigungen / Entlastungen</label><div id="tt80_reductions">${x.reductionEntries.map(tt80ReductionRow).join('')}</div><button type="button" class="chip" onclick="tt80AddReduction()">+ Ermäßigung</button><div class="tt81ReservedGrid"><div><label>Coaching (nur bei Coachfunktion)</label><input id="tt81_coaching_hours" type="number" min="0" step="0.5" value="${x.coachingHours}" oninput="tt81UpdateDeputatPreview()"><small>Standard: 1,5 h, frei in Lücken einplanbar</small></div><div><label class="check"><input id="tt81_team_active" type="checkbox" ${x.teamMeetingActive?'checked':''} onchange="tt81UpdateDeputatPreview()"> gemeinsame Teamstunde</label><input id="tt81_team_hours" type="number" min="0" step="0.5" value="${x.teamMeetingHours}" oninput="tt81UpdateDeputatPreview()"><small>Donnerstag, 5. Stunde · Standard: 0,5 h</small></div></div><div id="tt80_dep_preview" class="tt80Preview"><b>${x.fullDeputat} h</b> volles Deputat − <b>${red} h</b> Ermäßigung = <b>${teach} h</b> Unterricht · − <b>${ext} h</b> extern · − <b>${reserved} h</b> Coaching/Team · <strong>${kom} h durch KOMPASS</strong></div></div></details>
 <details open><summary>2 · Bereits verplante Einsätze und Zeitblocker</summary><div class="tt80Section"><p class="mini">Stunden in 8–10, Oberstufe, Themenfeld oder sonstigen Bereichen werden vom Deputat abgezogen. Bekannte Zeiten sperren den Generator verbindlich.</p><div class="tt80ExternalHead"><span>Bereich</span><span>Fach / Einsatz</span><span>Std.</span><span>Tag</span><span>Stunden</span></div><div id="tt80_external">${x.externalCommitments.map(tt80ExternalCommitmentRow).join('')}</div><button type="button" class="chip" onclick="tt80AddExternalCommitment()">+ Einsatz</button></div></details>
 <details open><summary>3 · Einsatzbereiche und Coachfunktion</summary><div class="tt80Section"><label>Einsatzstufen / Bereiche</label><div class="choice">${TT81_DEPLOYMENT_AREAS.map(a=>`<label class="check"><input type="checkbox" data-deployment="${a}" ${x.deploymentAreas.includes(a)?'checked':''}> ${a==='Oberstufe'?'Oberstufe':'Stufe '+a}</label>`).join('')}</div><div class="formgrid tt81CoachGrid"><div><label>Coach-Stufe</label><select id="tt81_coach_stage" onchange="tt81CoachStageChanged()"><option value="">kein Coach</option>${[5,6,7].map(y=>`<option value="${y}" ${x.coachStage===String(y)?'selected':''}>Stufe ${y}</option>`).join('')}</select></div><div><label>Coach-Farbteam</label><select id="tt81_coach_team" onchange="tt81CoachTeamChanged()"><option value="">kein Farbteam</option>${coachOptions.map(g=>`<option value="${esc(g.id)}" data-name="${esc(g.name)}" ${x.coachTeamId===g.id?'selected':''}>${esc(g.name)}</option>`).join('')}</select></div><div id="tt81_subteam_wrap" ${!/violett/i.test(x.coachTeamName||coachOptions.find(g=>g.id===x.coachTeamId)?.name||'')?'hidden':''}><label>Unterteam</label><select id="tt81_coach_subteam"><option value="">kein Unterteam</option><option value="Pink" ${x.coachSubteam==='Pink'?'selected':''}>Pink</option><option value="Lila" ${x.coachSubteam==='Lila'?'selected':''}>Lila</option></select><small>Team Violett: Pink (Schöne) / Lila (Brusda)</small></div></div></div></details>
 <details><summary>4 · Unterrichtszuordnungen</summary><div class="tt80Section"><p class="mini">Coachfunktion und Unterricht sind getrennt. Ein Coach kann Hauptfachunterricht in einem oder mehreren Teams übernehmen.</p><div class="ttTeamSubjectMatrix"><table><thead><tr><th>Team</th>${core.map(s=>`<th>${s}</th>`).join('')}</tr></thead><tbody>${groups.map(g=>`<tr><td>${esc(g.name)} · Stufe ${g.year}</td>${core.map(s=>{const k=tt73TeamKey(g.id,s);return `<td><input type="checkbox" data-team-subject="${esc(k)}" ${x.teamSubjects?.[k]?'checked':''}></td>`}).join('')}</tr>`).join('')}</tbody></table></div><div class="section">Weitere / geteilte Einsätze</div><div id="tt79_assignment_rows">${(x.teachingAssignments||[]).map(tt80AssignmentRow).join('')}</div><button type="button" class="chip" onclick="tt80AddAssignment()">+ Einsatz</button></div></details>
 <details open><summary>5 · Lernatelier je Stufe</summary><div class="tt80Section"><div class="tt81LAGrid">${[5,6,7].map(y=>tt81LAPanel(x,y)).join('')}</div></div></details>
 <details><summary>6 · Kreativband und Werkstatt</summary><div class="tt80Section"><div class="section">Kreativband</div><div class="formgrid"><div><label>Einsatzwunsch</label><select id="tt_creative_status"><option value="creative" ${x.creativeStatus==='creative'?'selected':''}>gerne im Kreativband</option><option value="maybe" ${x.creativeStatus==='maybe'?'selected':''}>nach Absprache</option><option value="la" ${x.creativeStatus==='la'?'selected':''}>lieber Lernatelier</option><option value="no" ${x.creativeStatus==='no'?'selected':''}>nicht einsetzen</option></select></div><div><label>Maximale Blöcke</label><input id="tt_creative_max" type="number" min="0" max="4" value="${x.creativeMaxBlocks}"></div></div><div class="tt76AreaGrid">${TT76_CREATIVE_AREAS.map(a=>`<label class="tt76Area"><input type="checkbox" data-creative-area="${esc(a)}" ${x.creativeAreas.includes(a)?'checked':''}><span>${esc(a)}</span></label>`).join('')}</div><label>Eigenes Angebot</label><input id="tt_creative_own" value="${esc(x.creativeOwnIdea||'')}"><label>Hinweise</label><textarea id="tt_creative_details">${esc(x.creativeDetails||'')}</textarea><label>Mögliche Tage</label><div class="choice">${[0,1,2,3].map(i=>`<label class="check"><input id="tt_creative_day_${i}" type="checkbox" ${x.creativeDays.includes(i)?'checked':''}> ${TT_DAYS[i]}</label>`).join('')}</div><div class="section">Werkstattunterricht / Projektphase</div><div class="formgrid"><div><label>Einsatzbereitschaft</label><select id="tt_workshop_status"><option value="yes" ${x.workshopStatus==='yes'?'selected':''}>gerne / möglich</option><option value="maybe" ${x.workshopStatus==='maybe'?'selected':''}>nach Absprache</option><option value="no" ${x.workshopStatus==='no'?'selected':''}>nicht einsetzen</option></select></div><div><label>Maximale Nachmittage</label><input id="tt_workshop_max_blocks" type="number" min="0" max="3" value="${x.workshopMaxBlocks}"></div><div><label>Arbeitsform</label><select id="tt81_workshop_tandem"><option value="egal" ${x.workshopTandem==='egal'?'selected':''}>egal</option><option value="allein" ${x.workshopTandem==='allein'?'selected':''}>bevorzugt allein</option><option value="tandem" ${x.workshopTandem==='tandem'?'selected':''}>bevorzugt im Tandem</option></select></div></div><label>Mögliche Nachmittage</label><div class="choice">${[0,1,3].map(i=>`<label class="check"><input id="tt_workshop_day_${i}" type="checkbox" ${x.workshopDays.includes(i)?'checked':''}> ${TT_DAYS[i]}</label>`).join('')}</div><label>Lab-Präferenzen</label><div class="tt76AreaGrid">${TT81_WORKSHOP_LABS.map(a=>`<label class="tt76Area"><input type="checkbox" data-workshop-lab="${esc(a)}" ${x.workshopLabs.includes(a)?'checked':''}><span>${esc(a)}</span></label>`).join('')}</div><label>Eigenes Lab / weiteres Angebot</label><input id="tt81_workshop_own" value="${esc(x.workshopOwnLab)}"><label>Wünsche / Hinweise</label><textarea id="tt_workshop_details">${esc(x.workshopDetails||'')}</textarea></div></details>
 <details><summary>7 · Verfügbarkeit und Optimierungswünsche</summary><div class="tt80Section"><label>Komplett freie / nicht verplanbare Tage</label><div class="choice">${TT_DAYS.map((d,i)=>`<label class="check"><input id="tt_full_day_${i}" type="checkbox" ${x.fullDayBlocked.includes(i)?'checked':''} onchange="tt73SetWholeDay(${i},this.checked)"> ${d}</label>`).join('')}</div><p class="mini">Feld anklicken: möglich → bevorzugt → vermeiden → gesperrt. Rot × ist eine verbindliche Sperrzeit.</p><div class="ttAvailGrid"><div></div>${Array.from({length:9},(_,i)=>`<b>${i+1}</b>`).join('')}${TT_DAYS.map((d,day)=>`<b>${d.slice(0,2)}</b>${Array.from({length:9},(_,i)=>{const p=i+1,s=x.availability?.[`${day}-${p}`]||'possible';return `<button type="button" class="ttAvail ${s}" data-day="${day}" data-period="${p}" data-state="${s}" onclick="tt72CycleAvailability(this)">${s==='preferred'?'★':s==='avoid'?'!':s==='blocked'?'×':'·'}</button>`}).join('')}`).join('')}</div><div class="formgrid"><div><label>Gewünschte freie Tage</label><select id="tt81_free_days"><option value="0" ${x.freeDayWish===0?'selected':''}>egal</option><option value="1" ${x.freeDayWish===1?'selected':''}>möglichst 1 freier Tag</option><option value="2" ${x.freeDayWish===2?'selected':''}>möglichst 2 freie Tage</option></select></div><div><label>Ankommensstunden maximal</label><input id="tt_arrival_max" type="number" min="0" max="5" value="${x.arrivalMax}"></div></div><label>Mögliche Tage für Ankommensstunde</label><div class="choice">${TT_DAYS.map((d,i)=>`<label class="check"><input id="tt_arrival_day_${i}" type="checkbox" ${x.arrivalDays.includes(i)?'checked':''}> ${d}</label>`).join('')}</div><label>Hinweise zur Ankommensstunde</label><textarea id="tt_arrival_notes">${esc(x.arrivalNotes||x.arrival||'')}</textarea></div></details>
 <details><summary>8 · Weitere Hinweise</summary><div class="tt80Section"><textarea id="tt_notes">${esc(x.notes||'')}</textarea></div></details>
 <div class="dialogActions"><button class="chip dark" onclick="ttSaveTeacher()">Speichern</button><button class="chip" onclick="State.dialog=null;renderDialog()">Abbrechen</button>${x.id?`<button class="chip" onclick="ttDeleteTeacher('${x.id}')">Löschen</button>`:''}</div></div></div>`;
}
function ttSaveTeacher(){
 const old=State.dialog?.id?tt().teachers.find(t=>t.id===State.dialog.id):null,x=tt81EnsureTeacher(old||tt78Teacher(null));x.id=x.id||uid('teacher');x.first=document.getElementById('tt_first').value.trim();x.last=document.getElementById('tt_last').value.trim();if(!x.first&&!x.last)return;
 x.fullDeputat=Number(document.getElementById('tt80_full_dep').value)||0;x.deputat=String(x.fullDeputat);x.subjects=document.getElementById('tt_subj').value.trim();x.otherSubjects=document.getElementById('tt_other_subj').value.trim();x.reductionEntries=[...document.querySelectorAll('.tt80ReductionRow')].map(r=>({id:uid('red'),label:r.querySelector('[data-red=label]').value.trim(),hours:Number(r.querySelector('[data-red=hours]').value)||0})).filter(r=>r.label||r.hours);x.reductions=String(tt80ReductionTotal(x));
 x.externalCommitments=[...document.querySelectorAll('.tt80ExternalRow')].map(r=>({id:uid('extc'),area:r.querySelector('[data-extc=area]').value,subject:r.querySelector('[data-extc=subject]').value.trim(),hours:Number(r.querySelector('[data-extc=hours]').value)||0,blocks:[...r.querySelectorAll('.tt80Block')].map(b=>({day:Number(b.dataset.day),periods:String(b.dataset.periods).split(',').map(Number).filter(Boolean)}))})).filter(e=>e.subject||e.hours||e.blocks.length);x.external=x.externalCommitments.flatMap(e=>e.blocks.flatMap(b=>b.periods.map(period=>({id:uid('external'),label:`${e.area}${e.subject?' · '+e.subject:''}`,day:b.day,period}))));
 x.deploymentAreas=[...document.querySelectorAll('[data-deployment]:checked')].map(e=>e.dataset.deployment);x.years=x.deploymentAreas.filter(a=>['5','6','7'].includes(a)).map(Number);x.coachStage=document.getElementById('tt81_coach_stage').value;const teamSel=document.getElementById('tt81_coach_team');x.coachTeamId=teamSel.value;x.coachTeamName=teamSel.options[teamSel.selectedIndex]?.dataset.name||teamSel.options[teamSel.selectedIndex]?.text||'';x.coachSubteam=document.getElementById('tt81_coach_subteam')?.value||'';x.coachTeams=x.coachTeamId?[x.coachTeamId]:[];x.coachingHours=Number(document.getElementById('tt81_coaching_hours').value)||0;x.teamMeetingActive=document.getElementById('tt81_team_active').checked;x.teamMeetingHours=Number(document.getElementById('tt81_team_hours').value)||0;
 x.teamSubjects={};document.querySelectorAll('[data-team-subject]').forEach(e=>x.teamSubjects[e.dataset.teamSubject]=e.checked);x.teachingAssignments=[...document.querySelectorAll('.tt79AssignmentRow')].map(r=>({year:Number(r.querySelector('[data-ta=year]').value),subject:r.querySelector('[data-ta=subject]').value.trim(),groupId:r.querySelector('[data-ta=group]').value,hours:Number(r.querySelector('[data-ta=hours]').value)||0})).filter(a=>a.subject&&a.hours>0);
 for(const y of [5,6,7])x.laByStage[y]={eligible:document.getElementById(`tt81_la_${y}_eligible`).checked,prefs:{1:Number(document.getElementById(`tt81_la_${y}_1`).value),2:Number(document.getElementById(`tt81_la_${y}_2`).value),3:Number(document.getElementById(`tt81_la_${y}_3`).value)}};x.laEligible=x.laByStage[6].eligible;x.laPrefs=x.laByStage[6].prefs;
 x.creativeStatus=document.getElementById('tt_creative_status').value;x.creativeMaxBlocks=Number(document.getElementById('tt_creative_max').value)||0;x.creativeAreas=[...document.querySelectorAll('[data-creative-area]:checked')].map(e=>e.dataset.creativeArea);x.creativeOwnIdea=document.getElementById('tt_creative_own').value.trim();x.creativeDetails=document.getElementById('tt_creative_details').value.trim();x.creativeDays=[0,1,2,3].filter(i=>document.getElementById('tt_creative_day_'+i).checked);
 x.workshopStatus=document.getElementById('tt_workshop_status').value;x.workshopMaxBlocks=Number(document.getElementById('tt_workshop_max_blocks').value)||0;x.workshopDays=[0,1,3].filter(i=>document.getElementById('tt_workshop_day_'+i).checked);x.workshopTandem=document.getElementById('tt81_workshop_tandem').value;x.workshopLabs=[...document.querySelectorAll('[data-workshop-lab]:checked')].map(e=>e.dataset.workshopLab);x.workshopOwnLab=document.getElementById('tt81_workshop_own').value.trim();x.workshopDetails=document.getElementById('tt_workshop_details').value.trim();x.workshop=x.workshopDetails;x.workshopMax=String(x.workshopMaxBlocks);
 x.fullDayBlocked=[0,1,2,3,4].filter(i=>document.getElementById('tt_full_day_'+i).checked);x.availability={};document.querySelectorAll('.ttAvail').forEach(b=>x.availability[`${b.dataset.day}-${b.dataset.period}`]=b.dataset.state);for(const d of x.fullDayBlocked)for(let p=1;p<=9;p++)x.availability[`${d}-${p}`]='blocked';x.freeDayWish=Number(document.getElementById('tt81_free_days').value)||0;x.arrivalMax=Number(document.getElementById('tt_arrival_max').value)||0;x.arrivalDays=[0,1,2,3,4].filter(i=>document.getElementById('tt_arrival_day_'+i).checked&&!x.fullDayBlocked.includes(i));x.arrivalNotes=document.getElementById('tt_arrival_notes').value.trim();x.notes=document.getElementById('tt_notes').value.trim();
 if(!old)tt().teachers.push(x);Store.save();State.dialog=null;toast('Lehrkraftprofil gespeichert');render();
}
function tt80KompassTarget(x){return tt81KompassTarget(x)}
function ttPickLACoach(lessons,year,day,period,laIndex,used=[]){return tt().teachers.map(tt81EnsureTeacher).filter(t=>(!t.years.length||t.years.includes(year))&&!used.includes(t.id)&&!ttUnavailable(t,day,period)&&!ttBusy(lessons,t.id,day,period)&&t.laByStage?.[year]?.eligible&&(t.laByStage[year].prefs?.[laIndex]??2)>0).sort((a,b)=>((b.laByStage[year].prefs?.[laIndex]??2)-(a.laByStage[year].prefs?.[laIndex]??2))||ttTeacherLoad(lessons,a.id)-ttTeacherLoad(lessons,b.id))[0]||null}
function ttTeachersView(){tt73SeedKnownAssignments();const q=ttTeacherFilter.toLowerCase(),rows=tt().teachers.map(tt81EnsureTeacher).filter(x=>!q||`${x.first} ${x.last} ${x.subjects} ${x.notes}`.toLowerCase().includes(q));return `<div class="grid2"><div class="card"><h2>IServ-Deputatswünsche importieren</h2><p>Importierte Angaben bleiben bearbeitbar. Deputat, externe Einsätze, Coaching, Lernatelier je Stufe und Werkstatt können anschließend ergänzt werden.</p><input type="file" accept=".csv,text/csv" onchange="ttImportTeachers(this.files[0])"><div class="mini">${tt().teachers.length} Lehrkräfte gespeichert${tt().importedAt?' · letzter Import '+new Date(tt().importedAt).toLocaleString('de-DE'):''}</div></div><div class="card"><h2>Lehrkraft ergänzen</h2><button class="chip dark" onclick="ttOpenTeacher()">Lehrkraft anlegen</button><input placeholder="Lehrkräfte durchsuchen …" value="${esc(ttTeacherFilter)}" oninput="ttTeacherFilter=this.value;render()"></div></div><div class="section">Lehrkräftepool</div><div class="teacherGrid tt72TeacherGrid">${rows.map(x=>{const target=tt81KompassTarget(x),areas=x.deploymentAreas||[],coach=x.coachStage?`Coach Stufe ${x.coachStage} · ${x.coachTeamName||tt().groups.find(g=>g.id===x.coachTeamId)?.name||'Team offen'}${x.coachSubteam?' / '+x.coachSubteam:''}`:'kein Coach';return `<div class="teacherCard tt77ClickableTeacher" role="button" tabindex="0" onclick="ttOpenTeacher('${x.id}')"><div class="teacherHead"><div><b>${esc(x.first)} ${esc(x.last)}</b><div class="mini">${esc(x.subjects||'Fächer noch offen')}</div></div><span class="badge">${target} h KOMPASS</span></div><div class="stageTags">${areas.slice(0,6).map(a=>`<span class="statusPill status-green">${a==='Oberstufe'?'Oberstufe':'Stufe '+a}</span>`).join('')}${areas.length>6?`<span class="statusPill status-empty">+${areas.length-6}</span>`:''}</div><div class="ttTeacherFacts"><span>Deputat ${x.fullDeputat} h</span><span>− ${tt80ReductionTotal(x)} h Ermäßigung</span><span>− ${tt80ExternalTotal(x)} h extern</span><span>− ${tt81ReservedTotal(x)} h Coaching/Team</span><span>${esc(coach)}</span><span>${x.freeDayWish?`Wunsch: ${x.freeDayWish} freie${x.freeDayWish===1?'r Tag':' Tage'}`:'keine Freitagsvorgabe'}</span></div><button class="chip" onclick="event.stopPropagation();ttOpenTeacher('${x.id}')">Profil bearbeiten</button></div>`}).join('')||'<div class="card empty">Noch keine Lehrkräfte importiert.</div>'}</div>`}
try{ttEnsureSchoolConfig().version='8.1a'}catch(e){}

/* ================================================================
   KOMPASS 8.1b – stundenweiser Organisationsgenerator
   Baut vollständig auf 8.1a auf. Alle Funktionen des Lehrkraftprofils
   bleiben erhalten; ausschließlich die Planungslogik wird erweitert.
   ================================================================ */

function tt81bTeacherTarget(t){
  try{return Math.max(0,Number(tt81KompassTarget(tt81EnsureTeacher(t)))||0)}catch(e){return 99}
}
function tt81bLoad(lessons,t){return lessons.filter(l=>l.teacherId===t.id&&!['Teamstunde'].includes(l.subject)).length}
function tt81bDayUsed(lessons,t,day){return lessons.some(l=>l.teacherId===t.id&&l.day===day)}
function tt81bAvailable(t,lessons,day,period){
  tt81EnsureTeacher(t);
  return !ttUnavailable(t,day,period)&&!ttBusy(lessons,t.id,day,period);
}
function tt81bAssignmentAllowed(t,group,subject){
  tt81EnsureTeacher(t);
  const key=tt73TeamKey(group.id,subject);
  if(t.teamSubjects&&t.teamSubjects[key])return true;
  return (t.teachingAssignments||[]).some(a=>Number(a.year)===Number(group.year)&&a.subject===subject&&(!a.groupId||a.groupId===group.id));
}
function tt81bCoreCandidates(group,subject,lessons,day,period){
  return tt().teachers.map(tt81EnsureTeacher).filter(t=>
    tt81bAssignmentAllowed(t,group,subject)&&tt81bAvailable(t,lessons,day,period)&&tt81bLoad(lessons,t)<tt81bTeacherTarget(t)
  ).sort((a,b)=>{
    const sa=tt81bTeacherSlotScore(a,lessons,day,period),sb=tt81bTeacherSlotScore(b,lessons,day,period);
    return sb-sa||tt81bLoad(lessons,a)-tt81bLoad(lessons,b);
  });
}
function tt81bTeacherSlotScore(t,lessons,day,period){
  let s=0,state=t.availability?.[`${day}-${period}`]||'possible';
  if(state==='preferred')s+=12;if(state==='avoid')s-=15;
  if(tt81bDayUsed(lessons,t,day))s+=Number(t.freeDayWish||0)*5; // Unterricht bündeln, freie Tage schützen
  s-=ttTeacherDayLoad(lessons,t.id,day)*2;
  return s;
}
function tt81bAdd(lessons,{year,subject,day,period,teacherId='',groupId='',roomId='',type='Unterricht',meta={}}){
  lessons.push({id:uid('lesson'),year:Number(year),subject,day:Number(day),period:Number(period),duration:1,teacherId,groupId,roomId,type,generated:true,meta});
}
function tt81bArrival(lessons,groups,issues){
  for(let day=0;day<5;day++)for(const group of groups){
    const candidates=tt().teachers.map(tt81EnsureTeacher).filter(t=>
      t.coachTeamId===group.id&&String(t.coachStage)===String(group.year)&&
      (t.arrivalDays||[]).includes(day)&&tt81bAvailable(t,lessons,day,1)&&
      lessons.filter(l=>l.teacherId===t.id&&l.subject==='Ankommen / Coaching').length<Number(t.arrivalMax||0)
    ).sort((a,b)=>tt81bLoad(lessons,a)-tt81bLoad(lessons,b));
    const teacher=candidates[0];
    tt81bAdd(lessons,{year:group.year,subject:'Ankommen / Coaching',day,period:1,teacherId:teacher?.id||'',groupId:group.id,type:'Ankommen',meta:{teamId:group.id}});
    if(!teacher)issues.push(`${group.name}: ${TT_DAYS[day]} 1. Stunde ohne verfügbaren zugeordneten Coach.`);
  }
}
function tt81bTeamMeeting(lessons,year,issues){
  const day=3,period=5,coaches=tt().teachers.map(tt81EnsureTeacher).filter(t=>String(t.coachStage)===String(year));
  let present=0;
  for(const t of coaches){
    if(tt81bAvailable(t,lessons,day,period)){
      tt81bAdd(lessons,{year,subject:'Teamstunde',day,period,teacherId:t.id,type:'Teamstunde',meta:{fixed:true,hours:.5}});present++;
    }
  }
  if(coaches.length&&present<coaches.length)issues.push(`Teamstunde Stufe ${year}: ${present} von ${coaches.length} Coaches können teilnehmen.`);
}
function tt81bCoreHours(year,subject){
  const req=tt().requirements.find(r=>Number(r.year)===Number(year)&&r.subject===subject);
  return Number(req?.hours)||3;
}
function tt81bGenerateCore(lessons,groups,issues){
  const subjects=['Deutsch','Mathematik','Englisch'];
  const remaining=new Map();
  for(const g of groups)for(const s of subjects)remaining.set(`${g.id}|${s}`,tt81bCoreHours(g.year,s));
  const slots=[];for(let day=0;day<5;day++)for(const period of [2,3,4])slots.push({day,period});
  // mehrere Umläufe: pro Slot höchstens ein Input je Fach, aber verschiedene Fächer dürfen parallel laufen
  let safety=300;
  while([...remaining.values()].some(v=>v>0)&&safety--){
    let best=null;
    for(const slot of slots){
      const subjectsAlready=new Set(lessons.filter(l=>l.day===slot.day&&l.period===slot.period&&ttIsCoreSubject(l.subject)).map(l=>l.subject));
      for(const g of groups){
        if(ttGroupBusy(lessons,g.id,slot.day,slot.period))continue;
        for(const s of subjects){
          const key=`${g.id}|${s}`,left=remaining.get(key)||0;if(left<=0||subjectsAlready.has(s))continue;
          // Fach möglichst über verschiedene Tage verteilen
          const sameDay=lessons.filter(l=>l.groupId===g.id&&l.subject===s&&l.day===slot.day).length;
          const groupDayLoad=lessons.filter(l=>l.groupId===g.id&&l.day===slot.day&&ttIsCoreSubject(l.subject)).length;
          const cand=tt81bCoreCandidates(g,s,lessons,slot.day,slot.period)[0];
          if(!cand)continue;
          let score=100-sameDay*45-groupDayLoad*10;
          score+=tt81bTeacherSlotScore(cand,lessons,slot.day,slot.period);
          // unterschiedliche Teams über die Woche streuen
          score-=lessons.filter(l=>l.day===slot.day&&l.period===slot.period&&ttIsCoreSubject(l.subject)).length*4;
          if(!best||score>best.score)best={g,s,key,slot,cand,score};
        }
      }
    }
    if(!best)break;
    tt81bAdd(lessons,{year:best.g.year,subject:best.s,day:best.slot.day,period:best.slot.period,teacherId:best.cand.id,groupId:best.g.id,type:'Unterricht'});
    remaining.set(best.key,(remaining.get(best.key)||0)-1);
  }
  for(const [key,left] of remaining)if(left>0){const [gid,s]=key.split('|'),g=groups.find(x=>x.id===gid);issues.push(`${g?.name||gid}: ${left} Stunde(n) ${s} konnten nicht passend gesetzt werden.`)}
}
function tt81bLA(lessons,groups,issues){
  const year=Number(groups[0]?.year),r=ttEnsureSchoolConfig().rules;
  const rooms=tt().rooms.filter(x=>String(x.type||x.name).toLowerCase().includes('lernatelier')).slice(0,3);
  for(let day=0;day<5;day++)for(const period of [2,3,4]){
    const inputs=new Set(lessons.filter(l=>l.year===year&&l.day===day&&l.period===period&&l.groupId&&ttIsCoreSubject(l.subject)).map(l=>l.groupId)).size;
    if(inputs>=groups.length)continue;
    const needed=Math.min(3,inputs>=Number(r.laReducedFromInputs||2)?Number(r.laReducedCoaches||2):Number(r.laNormalCoaches||3));
    const order=Number(r.la3MustBeStaffed)!==0?[3,1,2]:[1,2,3],used=[];
    for(const idx of order.slice(0,needed)){
      const coach=ttPickLACoach(lessons,year,day,period,idx,used);if(coach)used.push(coach.id);
      tt81bAdd(lessons,{year,subject:`Lernatelier ${idx}`,day,period,teacherId:coach?.id||'',roomId:rooms[idx-1]?.id||'',type:'Lernatelier',meta:{inputs,teamsInLA:groups.length-inputs}});
      if(!coach)issues.push(`Stufe ${year}: ${TT_DAYS[day]} ${period}. Stunde – Lernatelier ${idx} unbesetzt.`);
    }
  }
}
function tt81bCreative(lessons,year,issues){
  if(![5,6].includes(Number(year)))return;
  const teachers=tt().teachers.map(tt81EnsureTeacher).filter(t=>t.deploymentAreas?.includes(String(year))||t.years?.includes(Number(year)));
  for(const day of [0,1,2]){ // Do 5 ist Teamstunde; Freitag kein Kreativband
    const laUsed=[];let laCount=0;
    for(const t of teachers){
      if(![5,6].every(p=>tt81bAvailable(t,lessons,day,p)))continue;
      const currentBlocks=lessons.filter(l=>l.teacherId===t.id&&l.type==='Kreativband'&&l.period===5).length;
      const creative=(['creative','maybe'].includes(t.creativeStatus))&&(t.creativeDays||[]).includes(day)&&currentBlocks<Number(t.creativeMaxBlocks||0);
      if(creative){
        const label=tt75CreativeLabel(t);for(const p of [5,6])tt81bAdd(lessons,{year,subject:`Kreativband · ${label}`,day,period:p,teacherId:t.id,type:'Kreativband',meta:{block:'creative56'}});
      }else if(t.laByStage?.[year]?.eligible&&laCount<2){
        const pref=t.laByStage[year].prefs||{},idx=[3,1,2].filter(i=>(pref[i]||0)>0&&!laUsed.includes(i)).sort((a,b)=>(pref[b]||0)-(pref[a]||0))[0]||3;
        laUsed.push(idx);laCount++;for(const p of [5,6])tt81bAdd(lessons,{year,subject:`Lernatelier ${idx}`,day,period:p,teacherId:t.id,type:'Lernatelier',meta:{block:'creative56'}});
      }
    }
    if(!lessons.some(l=>l.year===year&&l.day===day&&l.period===5&&l.subject==='Lernatelier 3'))issues.push(`${TT_DAYS[day]} 5./6. Stunde: Lernatelier 3 ist nicht besetzt.`);
  }
}
function tt81bWorkshop(lessons,year,issues){
  if(![5,6].includes(Number(year)))return;
  const windows=[{day:0,periods:[8,9]},{day:1,periods:[8,9]},{day:3,periods:[7,8,9]}];
  const candidates=tt().teachers.map(tt81EnsureTeacher).filter(t=>['yes','maybe','gern','possible'].includes(String(t.workshopStatus).toLowerCase())||Number(t.workshopMaxBlocks)>0);
  for(const w of windows){
    const available=candidates.filter(t=>(t.workshopDays||[]).includes(w.day)&&w.periods.every(p=>tt81bAvailable(t,lessons,w.day,p)))
      .filter(t=>lessons.filter(l=>l.teacherId===t.id&&l.type==='Werkstatt'&&l.meta?.blockStart).length<Number(t.workshopMaxBlocks||1))
      .sort((a,b)=>(b.workshopLabs?.length||0)-(a.workshopLabs?.length||0)||tt81bLoad(lessons,a)-tt81bLoad(lessons,b));
    if(!available.length){issues.push(`${TT_DAYS[w.day]} Werkstatt: keine passende Lehrkraft verfügbar.`);continue;}
    // Angebote aus Präferenzen bilden; mindestens drei unterschiedliche Labs, sofern Personal vorhanden
    const usedLabs=new Set();
    for(const t of available.slice(0,Math.min(4,available.length))){
      const lab=(t.workshopLabs||[]).find(x=>!usedLabs.has(x))||t.workshopOwnLab||(t.workshopLabs||[])[0]||'Werkstatt-Lab';usedLabs.add(lab);
      for(const p of w.periods)tt81bAdd(lessons,{year,subject:`Werkstatt · ${lab}`,day:w.day,period:p,teacherId:t.id,type:'Werkstatt',meta:{lab,blockStart:p===w.periods[0]}});
    }
  }
}
function tt81bReport(lessons,groups,issues){
  // harte Prüfungen zusätzlich zum vorhandenen Prüfmodul
  for(const t of tt().teachers.map(tt81EnsureTeacher))for(const l of lessons.filter(x=>x.teacherId===t.id))if(ttUnavailable(t,l.day,l.period))issues.push(`${t.first} ${t.last}: trotz Sperrzeit am ${TT_DAYS[l.day]} in Stunde ${l.period} eingesetzt.`);
  for(let d=0;d<5;d++)for(const p of [2,3,4])for(const s of ['Deutsch','Mathematik','Englisch']){
    const n=lessons.filter(l=>l.day===d&&l.period===p&&l.subject===s&&l.year===Number(groups[0]?.year)).length;if(n>1)issues.push(`${TT_DAYS[d]} ${p}. Stunde: ${s} liegt ${n}-fach parallel.`);
  }
}
function ttGeneratePlan(){
  ttSeedSchoolModel();const p=ttActivePlan();if(!p)return alert('Bitte zuerst eine Planvariante anlegen.');
  if(p.lessons.length&&!confirm('Automatisch erzeugte Blöcke werden ersetzt; manuelle Einträge bleiben erhalten. Fortfahren?'))return;
  const lessons=p.lessons.filter(l=>!l.generated),issues=[],groups=tt().groups.filter(g=>Number(g.year)===Number(ttYear));
  if(!groups.length)return alert(`Für Stufe ${ttYear} sind noch keine Farbteams angelegt.`);
  tt81bArrival(lessons,groups,issues);
  tt81bGenerateCore(lessons,groups,issues);
  tt81bLA(lessons,groups,issues);
  tt81bTeamMeeting(lessons,Number(ttYear),issues);
  tt81bCreative(lessons,Number(ttYear),issues);
  tt81bWorkshop(lessons,Number(ttYear),issues);
  tt81bReport(lessons,groups,issues);
  p.lessons=lessons;tt().generator=tt().generator||{};tt().generator.lastRun=new Date().toISOString();tt().generator.report={issues:[...new Set(issues)],generated:lessons.filter(l=>l.generated).length,version:'8.1b'};
  try{ttEnsureSchoolConfig().version='8.1b'}catch(e){}
  Store.save();ttView='plan';toast('Stufenplan 8.1b berechnet');render();
}

// kompakte Ansicht um Werkstatt und Teamstunde ergänzt
function ttPlanView(p){
  if(!p)return '<div class="card empty">Noch keine Planvariante.</div>';
  const groups=tt().groups.filter(g=>Number(g.year)===Number(ttYear));
  return `<div class="card ttPlanToolbar"><select onchange="tt().activePlanId=this.value;Store.save();render()">${tt().plans.map(x=>`<option value="${x.id}" ${x.id===p.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select><select onchange="ttYear=Number(this.value);render()">${[5,6,7].map(y=>`<option value="${y}" ${ttYear===y?'selected':''}>Stufe ${y}</option>`).join('')}</select><button class="chip dark" onclick="ttGeneratePlan()">⚙ Stufenplan 8.1b berechnen</button></div><div class="ttCompactWrap"><table class="ttCompact"><thead><tr><th>Tag / Std.</th>${groups.map(g=>`<th>${esc(g.name.replace('Team ',''))}</th>`).join('')}<th class="laHead">Betreuung / Angebote</th></tr></thead><tbody>${TT_DAYS.map((dayName,day)=>Array.from({length:9},(_,i)=>i+1).map(period=>{const ls=p.lessons.filter(l=>l.day===day&&l.period===period&&Number(l.year)===Number(ttYear));const staffing=ls.filter(l=>!l.groupId);return `<tr class="${period===1?'arrivalRow':''} ${[5,6].includes(period)?'creativeRow':''}"><th>${dayName.slice(0,2)} ${period}</th>${groups.map(g=>{const l=ls.find(x=>x.groupId===g.id);return `<td onclick="ttOpenLesson('${l?.id||''}',${day},${period})">${l?tt72CellText(l):'<span class="emptyDot">·</span>'}</td>`}).join('')}<td class="laCell">${staffing.map(l=>`<span class="laMini ${l.teacherId?'ok':'open'} ${l.type==='Kreativband'?'creativeMini':''}">${esc(l.subject)}: ${esc(ttNameTeacher(l.teacherId).replace('Unbekannt','offen'))}</span>`).join('')||'<span class="emptyDot">·</span>'}</td></tr>`}).join('')).join('')}</tbody></table></div>`;
}
