...
const Store = {
  ...
  load(){
    ...
    this.data = {version:'4.1',subjects:clone(SEED.subjects),coreSubjects:clone(SEED.coreSubjects),sprints:clone(SEED.sprints),pupils:clone(SEED.pupils),competencies:clone(SEED.competencies),records:{},lebDrafts:{},settings:{scoreConfig:{...DEFAULT_SCORE_CONFIG}},metadata:{createdAt:new Date().toISOString(),version:'4.1'}};
    this.save();
  },
  migrate(){
    const d=this.data;
...
Store.load();
Object.defineProperties(Store,{sprints:{get(){return this.data.sprints},set(v){this.data.sprints=v}},subjects:{get(){return this.data.subjects},set(v){this.data.subjects=v}},coreSubjects:{get(){return this.data.coreSubjects},set(v){this.data.coreSubjects=v}},pupils:{get(){return this.data.pupils},set(v){this.data.pupils=v}},competencies:{get(){return this.data.competencies},set(v){this.data.competencies=v}},records:{get(){return this.data.records},set(v){this.data.records=v}},lebDrafts:{get(){return this.data.lebDrafts},set(v){this.data.lebDrafts=v}},settings:{get(){return this.data.settings},set(v){this.data.settings=v}},metadata:{get(){return this.data.metadata},set(v){this.data.metadata=v}}});
