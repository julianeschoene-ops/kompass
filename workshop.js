function workshop(){
  ...
  if(comp){
    content += `<div class="card"><b>${comp.text}</b><span class="badge">${comp.kind==='prozess'?'Prozesskompetenz':'Fachkompetenz'}</span><p>${comp.subject} · Sprint ${comp.sprint} · ${comp.area} · Niveau ${comp.level}</p></div>`;
  } else {
    content += `<div class="card"><p>Für dieses Fach und diesen Sprint sind noch keine Kompetenzen angelegt.</p></div>`;
  }
...
function renderDialog(){
  ...
  target.innerHTML = `<div class="dialogBackdrop">
    <div class="dialog">
      ...
      <div class="card" style="box-shadow:none"><b>${comp.text}</b><span class="badge">${comp.kind==='prozess'?'Prozess':'Fach'}</span><p>${comp.subject} · ${comp.area} · Niveau ${comp.level}</p></div>
