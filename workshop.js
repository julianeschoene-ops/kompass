...
function renderPupilTable(comp){
  ...
  const rows = filteredPupils().map(p=>{
    ...
    return `<tr>
      ...
      <td>${statusLabel(r.current)}${r.score ? `<div class="mini">${r.score.points}/${r.score.max} P. · ${r.score.percent}%</div>` : ''}</td>
...
function renderDialog(){
  ...
  target.innerHTML = `<div class="dialogBackdrop">
    <div class="dialog">
      ...
      <label>Punkte-Auswertung</label>
      <div class="scoreBox">
        <div><label>Erreichte Punkte</label><input id="scorePoints" type="number" min="0" step="0.5" value="${record.score?.points ?? ''}" oninput="updateScoreStatus()" placeholder="z. B. 12"></div>
        <div><label>Maximalpunkte</label><input id="scoreMax" type="number" min="1" step="0.5" value="${record.score?.max ?? ''}" oninput="updateScoreStatus()" placeholder="z. B. 20"></div>
        <div><label>Rot bis %</label><input id="scoreRedMax" type="number" min="0" max="100" value="${record.score?.redMax ?? comp.scoreConfig?.redMax ?? 49}" oninput="updateScoreStatus()"></div>
        <div><label>Gelb bis %</label><input id="scoreYellowMax" type="number" min="0" max="100" value="${record.score?.yellowMax ?? comp.scoreConfig?.yellowMax ?? 74}" oninput="updateScoreStatus()"></div>
      </div>
      <div id="scorePreview" class="mini">${record.score ? `Automatisch berechnet: ${record.score.percent}% → ${statusPlain(record.current)}` : 'Optional: Punkte eintragen, dann wird rot/gelb/grün automatisch gesetzt.'}</div>
...
function statusPlain(value){ if(value==='green') return 'erreicht'; if(value==='yellow') return 'teilweise erreicht'; if(value==='red') return 'noch nicht erreicht'; return 'nicht bewertet'; }
function updateScoreStatus(){
  const points=Number(document.getElementById('scorePoints')?.value);
  const max=Number(document.getElementById('scoreMax')?.value);
  const redMax=Number(document.getElementById('scoreRedMax')?.value || 49);
  const yellowMax=Number(document.getElementById('scoreYellowMax')?.value || 74);
  const preview=document.getElementById('scorePreview');
  if(!max || Number.isNaN(points)){ if(preview) preview.textContent='Optional: Punkte eintragen, dann wird rot/gelb/grün automatisch gesetzt.'; return; }
  const percent=Math.round((points/max)*100);
  const status=scoreToStatus(percent, redMax, yellowMax);
  const key=recKey(State.dialog.competencyId,State.dialog.pupilId);
  Store.records[key]=Store.records[key]||{current:null,level:null,firstGreen:null,history:[],notes:[]};
  Store.records[key].current=status;
  Store.records[key].score={points,max,percent,redMax,yellowMax};
  if(preview) preview.textContent=`Automatisch berechnet: ${percent}% → ${statusPlain(status)}`;
  renderDialog();
}
...
function saveAssessment(){
  ...
  const pointsRaw = document.getElementById('scorePoints')?.value;
  const maxRaw = document.getElementById('scoreMax')?.value;
  const redMax = Number(document.getElementById('scoreRedMax')?.value || 49);
  const yellowMax = Number(document.getElementById('scoreYellowMax')?.value || 74);
  if(pointsRaw !== '' && maxRaw !== ''){
    const points = Number(pointsRaw), max = Number(maxRaw);
    if(max > 0){
      const percent = Math.round((points/max)*100);
      record.score = {points,max,percent,redMax,yellowMax};
      record.current = scoreToStatus(percent, redMax, yellowMax);
    }
  }
  ...
  record.history.push({date:today(),teacher:State.teacher,value:status,level,note,score:record.score||null});
