-- KOMPASS 8.3.8 – Kontenverwaltung + Coachgruppen
-- Einmal vollständig im Supabase SQL Editor ausführen.

-- Profilfelder sicherstellen
...
-- Auth-Trigger: jedes neue Auth-Konto erhält automatisch ein KOMPASS-Profil.
...
-- Bereits vorhandene Auth-Konten ohne Profil reparieren.
insert into public.kompass_profiles(id,display_name,role,active,coach_teams,coaching_groups)
select u.id,coalesce(u.raw_user_meta_data->>'display_name',u.email,''),'teacher',false,'{}'::jsonb,'{}'::jsonb
from auth.users u
left join public.kompass_profiles p on p.id=u.id
where p.id is null
on conflict (id) do nothing;

-- RLS für die Admin-Verwaltung konsistent setzen.
